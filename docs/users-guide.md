...
`#[...]` style tags so it can be embedded directly in the tmux status line. For
the change from the pre-0.2 usage to this CLI, see the
...
Setting the length to 999 only raises tmux's own display-width cap; it does not
stop tmux from redrawing, and content can still be truncated on narrow clients
where the terminal width is the limiting factor. Installing with
`--position right` also enables the clock by passing `--show-clock true` to the
status command.
...
`cmds.status`, `cmds.refresh`, or `cmds.install` in the config file.
Environment variables use the `DBAR_CMDS_STATUS_`, `DBAR_CMDS_REFRESH_`, or
`DBAR_CMDS_INSTALL_` prefixes.
...
dbar degrades silently by design: a failed `git` or `tmux` probe never breaks
the rendered status line, it just omits or simplifies the affected segment. To
inspect what was absorbed, set `DBAR_DIAGNOSTICS` to any value or pass
`--diagnostics` to `status` or `refresh`. Diagnostics are written to stderr,
one line per failure, using fixed operation labels and bounded outcome
categories. They do not include raw URLs, paths, filenames, or command stderr.
Diagnostics are opt-in and never change stdout: the status line is identical
whether or not diagnostics are enabled, so this is safe inside a tmux status
command.

## Status outcomes and defaults

`status` defaults to the current working directory, reads a cached pull-request
number when one is fresh, leaves the clock disabled, and uses `%H:%M` when the
clock is enabled. Supplying `--project-dir` points the git probes at another
directory; tmux session, window, pane, and socket values can be supplied
explicitly or resolved from the current tmux server.

On a missing, expired, or unreadable PR cache entry, `status` omits the PR
segment. Run `dbar refresh` to perform the live GitHub lookup and update the
cache; `status` remains a read-only query and never invokes GitHub or mutates
cache files. Missing or failed git and tmux probes are fallbacks rather than
fatal status errors. An invalid clock format or invalid configuration value is
a configuration error and stops the command instead of silently choosing a
...
`refresh` uses the current directory by default and accepts `--project-dir`,
`--cache-dir`, and `--pr-cache-ttl-seconds`. It writes no normal output on a
successful refresh; use `--diagnostics` or `DBAR_DIAGNOSTICS` to inspect
failures on stderr.
...
unreadable, or malformed entry causes `status` to omit the PR segment until
`dbar refresh` performs a live lookup. A successful GitHub number, an explicit
no-PR result, or a `pr/<number>`/`pr-<number>` style branch fallback is cached;
a failed GitHub lookup is not cached, so a transient failure does not persist
for the whole TTL. If the cache directory cannot be resolved or written,
`refresh` reports a bounded diagnostic and `status` continues to render without
a PR value.
